{
  ...
  class GridFx {
    constructor(el) {
      ...
      this.items = [];
      Array.from(this.DOM.el.querySelectorAll(".grid__item")).forEach((item) =>
        this.items.push(new GridItem(item))
      );
      // The nav link that will trigger the loop effect.
      this.DOM.showPhotosCtrl = document.querySelector(
        'nav.menu > .menu__item[href="#content-Projects"]'
      );
      // The content elements.
      this.DOM.content = {
        main: document.querySelector(".content--main"),
        Projects: document.querySelector(".content--Projects"),
    ...
    initEvents() {
      // Mouse events for the "Projects" link.
    ...
    loopThroughPhotos() {
      ...
      for (let item of this.items) {
        if (inViewport(item.DOM.wrap)) {
          ...
          const scaleVal =
            ratioWrap > ratioWindow
              ? winsize.width / itemRect.width
              : winsize.height / itemRect.height;
      ...
      if (inViewportItems.length > 1) {
        ...
        const loop = (pos = 0) => {
          ...
          if (oldpos === -1) {
            ...
            TweenMax.set(this.currentItem.DOM.wrap, {
              opacity: 1,
              zIndex: 998,
            });
          }
      ...
      for (let item of this.items) {
        if (item == this.currentItem) {
          ...
          TweenMax.to(item.DOM.wrap, 0.2, {
            ...
            // Reset.
            onComplete: () =>
              TweenMax.set(item.DOM.wrap, { x: 0, y: 0, scale: 1 }),
          });
        } else {
          // Reset.
    ...
    showGrid() {
      ...
      if (this.currentItem) {
        ...
        side =
          itemRect.left + itemRect.width / 2 < winsize.width / 2
            ? "left"
            : "right";
      }
    ...
    toggleContent(isGrid = true) {
      ...
      this.DOM.content.main.classList[isGrid ? "remove" : "add"](
        "content--current"
      );
      this.DOM.content.Projects.classList[isGrid ? "add" : "remove"](
        "content--current"
      );
    }
  }
